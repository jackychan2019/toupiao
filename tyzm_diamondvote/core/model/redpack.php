...
class Tyzm_redpack{
	...
	public $tableviporder = 'tyzm_diamondvote_viporder';
	 ...
	 public function sendredpack($redpackid,$rid,$type='0')
    {
        global $_W;
        $rid          = intval($rid);
        ...
        if (!empty($lickylog)){
            if ($lickylog['result_code'] != 'SUCCESS') {
                ...
                $data['send_name']    = $type=='1'?$this->str_cut($configdata['send_namejoin'], 32, ''):$this->str_cut($configdata['send_name'], 32, '');
                $data['act_name']     = $type=='1'?$this->str_cut($configdata['act_namejoin'], 32, ''):$this->str_cut($configdata['act_name'], 32, '');
                $data['remark']       = $type=='1'?$this->str_cut($configdata['remarkjoin'], 256, ''):$this->str_cut($configdata['remark'], 256, '');
                $data['wishing']      = $type=='1'?$this->str_cut($configdata['wishingjoin'], 128, ''):$this->str_cut($configdata['wishing'], 128, '');
                $data['total_num']    = 1;
