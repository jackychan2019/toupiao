...
if($ty=='addvotenum'){
		$votenum = intval($_GPC['votenum']);
		if(pdo_update($this->tablevoteuser,array('votenum +='=>$votenum),array('id' =>$id,'rid' =>$rid,'uniacid'=>$uniacid))){
			$out['status'] = 200;
		    exit(json_encode($out));
		}else{
			$out['status'] = 0;
...
if($ty=='deleteactivity'){

	$act=pdo_delete($this->tablereply,array('rid' => $rid,'uniacid' => $uniacid));
	if($act){
		//选手数据 
		pdo_delete($this->tablevoteuser,array('rid' => $rid,'uniacid' => $uniacid));
		//删除用户数据
		pdo_delete($this->tablevotedata,array('rid' => $rid,'uniacid' => $uniacid));
		//删除礼物数据
		pdo_delete($this->tablegift,array('rid' => $rid,'uniacid' => $uniacid));
		//删除访问数据
		pdo_delete($this->tablecount,array('rid' => $rid,'uniacid' => $uniacid));
	}

	message('删除成功！', $this->createWebUrl('manage', array('name' => 'tyzm_diamondvote')), 'success');
}