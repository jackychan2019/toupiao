...
//$type = in_array(, array('image','audio','video')) ? $_GPC['type'] : 'image';
...
if ($do == 'upload') {
	...
	if(strpos($type,'image')!==false){
		...
		if (!empty($_FILES['file']['name'])) {
			...
			file_put_contents(ATTACHMENT_ROOT."/video.txt",$fullname);
...
//
	...
	if(strpos($type,'video')!==false){
		...
		$file = file_upload($_FILES['file'],'video');
		...
		if (!empty($_FILES['file']['name'])) {
			...
			$result['error']['message'] = '请选择要上传的视频！';
			die(json_encode($result));
		}
