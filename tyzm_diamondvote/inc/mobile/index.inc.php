...
$reply = pdo_fetch("SELECT rid,title,topimg,eventrule,thumb,sharetitle,shareimg,sharedesc,addata,linkdata,config,style,endtime,apstarttime,apendtime,status,description FROM " . tablename($this->tablereply) . " WHERE rid = :rid ORDER BY `id` DESC", array(':rid' => $rid));
