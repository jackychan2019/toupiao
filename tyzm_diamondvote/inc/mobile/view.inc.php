...
$reply = pdo_fetch("SELECT rid,title,eventrule,sharedesc,description,config,style,area,addata,linkdata,applydata,endtime,apstarttime,apendtime,status FROM " . tablename($this->tablereply) . " WHERE rid = :rid ", array(':rid' => $rid));
...
foreach ($applydata as $key=> $row) {
	...
	if($row['infotype']=='vqqcom'){
		$videourl=$joindata[$key]['val'];
	}
}


	if(!empty($videourl)){//格式化腾讯视频
		$urlarr=explode("/", $videourl);
		foreach ($urlarr as $key=> $row) {
			if(strpos($row,'.html') !==false){
				$vkey=str_replace('.html','',$row);
				$videourl="https://v.qq.com/iframe/player.html?vid=".$vkey."&tiny=0&auto=0";
				break;
			}
		}
	}
...
if($reply['isgiftad']==1){
	$reply['voteadurl']=$_W['siteroot']."app/".$this->createMobileUrl('gift', array('id' => $id,'rid' => $rid));;
}
...
$_share['link'] = $_W['siteroot']."app/".$this->createMobileUrl('view', array('id' => $id,'rid' => $rid,'isopenlink'=>'first'));
$_W['page']['sitename']="我是".$voteuser['name'].",编号".$voteuser['noid'].",正在参加".$reply['title']."活动，来帮我投一票吧。";

include $this->template(m('tpl')->style('view',$reply['style']['template']));
